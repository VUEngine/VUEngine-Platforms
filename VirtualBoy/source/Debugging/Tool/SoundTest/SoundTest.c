...
void SoundTest::printGUI(bool clearScreen)
{
	...
	if(NULL == _userSounds[this->soundIndex])
	{
		Printer::text("No sounds found", 1, 4, NULL);
		Printer::text("Define some in _userSounds global variable", 1, 6, NULL);
		return;
	}
	...
	if(!Sound::isPlaying(this->sound))
	{
		Printer::text("Play     \x13", xControls, yControls++, NULL);
	}
	else
	{
		Printer::text("Pause    \x13", xControls, yControls++, NULL);
	}
	Printer::text("Rewind   \x14", xControls, yControls++, NULL);
	yControls++;
	Printer::text("T.Freq. \x1F\x1A", xControls, yControls++, NULL);
	Printer::text("T.Scale \x1F\x1B", xControls, yControls++, NULL);
	Printer::text("T.Res. \x1F\x1C\x1D", xControls, yControls++, NULL);
