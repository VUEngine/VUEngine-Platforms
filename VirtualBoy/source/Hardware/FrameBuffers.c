...
static bool FrameBuffers::shrinkLineToScreenSpace
(
	fixed_ext_t* x0, fixed_ext_t* y0, fixed_ext_t* parallax0, fixed_ext_t dx, fixed_ext_t dy, 
	fixed_ext_t dParallax, fixed_ext_t x1, fixed_ext_t y1, fixed_ext_t parallax1
)
{
	fixed_ext_t x = *x0;
	fixed_ext_t y = *y0;
	...
	NM_ASSERT(0 != dx, "FrameBuffers::shrinkLineToScreenSpace: dx = 0");
...
{	
	if(interlaced)
	{
		...
		FrameBuffers::drawColorPixelInterlaced((uint8*)buffer, point.x, point.y, 0!= bufferIndex ? -point.parallax : point.parallax, color);
	}
	else
	{
		...
		if(!FrameBuffers::isPointInsideFrustum(point))
		{
			return false;
		}

		FrameBuffers::drawColorPixel((uint8*)leftBuffer, (uint8*)rightBuffer, point.x, point.y, point.parallax, color);
	}
...
{	
	...
	)
	{
		...
		if
		(
			!FrameBuffers::shrinkLineToScreenSpace
			(
...
*/
	...
	_drawnPixelsCounter += totalPixels;

	if(_drawnPixelsCounter > _maximumPixelsToDraw)
	{
		return false;
	}
	...
	if(interlaced)
	{
		...
		if(0 != bufferIndex)
		{
			...
			{
				FrameBuffers::drawColorPixelInterlaced
				(
	...
	else
	{
		...
		FrameBuffers::drawColorPixel((uint8*)leftBuffer, (uint8*)rightBuffer, -1, -1, 0, 0);
#endif
	}
...
static bool FrameBuffers::drawCircle(PixelVector center, int16 radius, int32 color, uint8 bufferIndex, bool interlaced)
{
	if(0 >= radius)
	{
		return false;
	...
	if(interlaced && 3 < radius)
	{
		...
		while (x >= y)
		{
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, center.x + x + radius, center.y + y + radius, center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, center.x + y + radius, center.y + x + radius, center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, center.x - x + radius, center.y + y + radius, center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, center.x - y + radius, center.y + x + radius, center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, center.x - x + radius, center.y - y + radius, center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, center.x - y + radius, center.y - x + radius, center.parallax, color);
			...
			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, center.x + x + radius, center.y + y + radius, -center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, center.x + y + radius, center.y + x + radius, -center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, center.x - x + radius, center.y + y + radius, -center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, center.x - y + radius, center.y + x + radius, -center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, center.x - x + radius, center.y - y + radius, -center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, center.x - y + radius, center.y - x + radius, -center.parallax, color);
...
static bool FrameBuffers::drawX(PixelVector center, int16 length, int32 color, uint8 bufferIndex, bool interlaced)
{
	if(!FrameBuffers::isPointInsideFrustum(center))
	{
		return false;
	}
	...
	if(interlaced)
	{
		...
		{
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, x, y, center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, (center.x + halfLength) - lengthHelper, y, center.parallax, color);

			x++;
			y++;
			lengthHelper++;

			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, x, y, -center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, (center.x + halfLength) - lengthHelper, y, -center.parallax, color);
		}
	}
	else
	{
		...
		for(; lengthHelper <= length; lengthHelper++, x++, y++)
		{
			FrameBuffers::drawColorPixel((uint8*)leftBuffer, (uint8*)rightBuffer, x, y, center.parallax, color);
...
static bool FrameBuffers::drawCross(PixelVector center, int16 length, int32 color, uint8 bufferIndex, bool interlaced)
{
	if(!FrameBuffers::isPointInsideFrustum(center))
	{
		return false;
	...
	if(interlaced)
	{
		...
		{
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, center.x + coordinate, center.y, center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)leftBuffer, center.x, center.y + coordinate, center.parallax, color);

			coordinate++;
			lengthHelper++;

			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, center.x + coordinate, center.y, -center.parallax, color);
			FrameBuffers::drawColorPixelInterlaced((uint8*)rightBuffer, center.x, center.y + coordinate, -center.parallax, color);
		}
	}
	else
	{
		...
		for(int16 coordinate = -halfLength; lengthHelper < length; coordinate++, lengthHelper++)
		{
			FrameBuffers::drawColorPixel((uint8*)leftBuffer, (uint8*)rightBuffer, center.x + coordinate, center.y, center.parallax, color);
			FrameBuffers::drawColorPixel((uint8*)leftBuffer, (uint8*)rightBuffer, center.x, center.y + coordinate, center.parallax, color);
		}
	}
...
static bool FrameBuffers::drawSolidCircle(PixelVector center, int16 radius, int32 color, uint8 bufferIndex, bool interlaced)
{
	if(0 >= radius)
	{
		return false;
...
static bool FrameBuffers::drawSolidRhumbus(PixelVector center, int16 radius, int32 color, uint8 bufferIndex, bool interlaced)
{
	if(!FrameBuffers::isPointInsideFrustum(center))
	{
		return false;
...
/// This will yield no result for color = 0, so for drawing a black pixel, use FrameBuffers_drawBlackPixel
/// instead.
...
static void FrameBuffers::drawPixel(uint32 buffer, uint16 x, uint16 y, int32 color)
{
...
/// @param y			Camera y coordinate
/// @param color		The color to draw (__COLOR_BRIGHT_RED, __COLOR_MEDIUM_RED or __COLOR_DARK_RED)
static void FrameBuffers::drawColorPixel(uint8* leftBuffer, uint8* rightBuffer, int16 x, int16 y, int16 parallax, int32 color)
{
#ifdef __DIRECT_DRAW_OPTIMIZED_VERTICAL_LINES
...
static void FrameBuffers::drawColorPixelInterlaced(uint8* buffer, int16 x, int16 y, int16 parallax, int32 color)
{
	// Calculate pixel position
	// Each column has 16 words, so 16 * 4 bytes = 64, each byte represents 4 pixels
	uint16 displacement = ((x - parallax) << 6) + (y >> 2);
...
/// @param x			Camera x coordinate
/// @param y			Camera y coordinate
static void FrameBuffers::drawBlackPixel(uint8* leftBuffer, uint8* rightBuffer, int16 x, int16 y, int16 parallax)
{
	uint16 yHelper = y >> 2;
	uint8 pixel = ~(3 << ((y & 3) << 1));
...
static bool FrameBuffers::isPointInsideFrustum(PixelVector point)
{
...
static void FrameBuffers::reset()
{
	FrameBuffers::getInstance();

	_currentDrawingFrameBufferSet = 0;
	_drawnPixelsCounter = 0;
	_maximumPixelsToDraw = __DIRECT_DRAW_MAXIMUM_NUMBER_OF_PIXELS;

	FrameBuffers::setFrustum((CameraFrustum)
	{
...
static void FrameBuffers::setFrustum(CameraFrustum frustum)
{
...
static CameraFrustum FrameBuffers::getFrustum()
{
	return _frustum;
}
...
#ifndef __SHIPPING
static void FrameBuffers::print(int16 x, int16 y)
{
	Printer::text("DIRECT DRAW", x, y++, NULL);
	y++;
	Printer::text("Drawn pixels:      ", x, y, NULL);
	Printer::int32(_drawnPixelsCounter, x + 14, y++, NULL);
	Printer::text("Max. pixels:       ", x, y, NULL);
	Printer::int32(_maximumPixelsToDraw, x + 14, y++, NULL);
}
#endif
...
bool FrameBuffers::onEvent(ListenerObject eventFirer, uint16 eventCode)
{
	switch(eventCode)
	{
		case kEventDisplayUnitGameStart:
		{
			_currentDrawingFrameBufferSet = DisplayUnit::getCurrentDrawingFrameBufferSet(eventFirer);
		...
		case kEventDisplayUnitVBlank:
		{
			...
			if(_drawnPixelsCounter <= _maximumPixelsToDraw)
			{
				_maximumPixelsToDraw += __DIRECT_DRAW_MAXIMUM_NUMBER_OF_PIXELS_RECOVERY;

				if(__DIRECT_DRAW_MAXIMUM_NUMBER_OF_PIXELS < _maximumPixelsToDraw)
				{
					_maximumPixelsToDraw = __DIRECT_DRAW_MAXIMUM_NUMBER_OF_PIXELS;
				}
			}
		...
		case kEventDisplayUnitGameStartDuringVBlank:
		{
			_maximumPixelsToDraw = _drawnPixelsCounter - __DIRECT_DRAW_MAXIMUM_NUMBER_OF_PIXELS_OVERHEAD;

			if(__DIRECT_DRAW_MINIMUM_NUMBER_OF_PIXELS > _maximumPixelsToDraw)
			{
				_maximumPixelsToDraw = __DIRECT_DRAW_MINIMUM_NUMBER_OF_PIXELS;
			}
...
void FrameBuffers::constructor()
{
	...
	DisplayUnit::addEventListener(DisplayUnit::getInstance(), ListenerObject::safeCast(this), kEventDisplayUnitGameStartDuringVBlank);
}

//——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————

void FrameBuffers::destructor()
{
	...
	DisplayUnit::removeEventListener(DisplayUnit::getInstance(), ListenerObject::safeCast(this), kEventDisplayUnitGameStartDuringVBlank);
